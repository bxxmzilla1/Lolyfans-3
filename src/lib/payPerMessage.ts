 ...
 * messages spend that credit first. Once it's gone they must have a verified
 * card to keep chatting. Further costs accrue on a per-chat owed balance that
 * is auto-charged to their card roughly once an hour.
 */
export type PayPerMessage = {
  enabled: boolean;
  /** Price of one fan message, in cents. */
  priceCents: number;
  /** Free money credited to each fan's balance when they accept, in cents. */
