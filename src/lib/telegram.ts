 ...
 *
 * Deliberately NOT pooled: holding a shared connection open across requests
 * makes several serverless instances use the same auth key from different
 * IPs at once, and Telegram kills that with AUTH_KEY_DUPLICATED (which can
 * invalidate the whole session). Short-lived connect → call → disconnect is
