...
/**
 * Send any link into the creator's Saved Messages as bold clickable words
 * (HTML), so it can be copied into DMs or channels without a raw URL.
 */
export async function POST(req: NextRequest) {
  ...
  const label =
    String(body.text || "").trim().slice(0, 80) || "Tap here";
