...
export async function POST(req: NextRequest) {
  ...
  if (cmd === "/start" || cmd === "/help") {
    ...
    await sendTelegram(
      ...
      enrolled
        ...
        : "👋 <b>Lolyfans admin bot</b>\n\nSend the admin code to get notified whenever a fan signs up, verifies a card, or starts chatting with another creator — and which creator it was."
    );
  ...
  if (text === ADMIN_NOTIFY_CODE) {
    ...
    await sendTelegram(
      chatId,
      added
        ? "✅ <b>Admin access granted.</b>\n\nYou'll now be notified — for all creators — about every new signup, every first card verification, and whenever a fan starts chatting with another creator (Message button or creator link).\n\n" +
            HELP
